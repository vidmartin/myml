...
class TensorNode(ABC):
    ...
    def get_gradients_against(self, leaves: list[ConstantNode]) -> list[np.ndarray]:
        ...
        for dep, der in zip(self.get_direct_dependencies(), self.get_derivatives()):
            temp_gradients = dep.get_gradients_against(leaves_of_interest)
    ...
    def get_value(self):
        if self._value is None:
            ...
            if np.isnan(self._value).any():
                ...
                if isinstance(self, ElementwiseMulNode):
                    lhs_v = self._deps[0].get_value()
                    rhs_v = self._deps[1].get_value()
                    print(f"lhs isnan corresponding: {lhs_v[np.isnan(self._value)]}")
                    print(f"rhs isnan corresponding: {rhs_v[np.isnan(self._value)]}")
        return self._value
    @abstractmethod
...
def softmax_node(dep: TensorNode):
    ...
    dep_sh = dep.get_shape()
    exp = ElementwiseUnaryNode(
        dep,
        elementwise.CommonElementwiseOperations.exp()
    )
    # print(f"exp shape: {exp.get_shape()}")
    rowsum = TensorDotNode(
        exp,
        ConstantNode(np.ones(dep_sh[-1])),
        1
    )
    # print(f"rowsum shape: {rowsum.get_shape()}")
    rowsum_expanded = TransposeNode(
        ExtendNode(rowsum, (dep_sh[-1],)),
        tuple(range(len(dep_sh)))[1:] + (0,)
    )
    # print(f"rowsum_expanded shape: {rowsum_expanded.get_shape()}")
    rowsum_expanded_inverted = ElementwiseUnaryNode(
        rowsum_expanded,
        elementwise.CommonElementwiseOperations.ipow(-1)
    )
    # print(f"rowsum_expanded_inverted shape: {rowsum_expanded_inverted.get_shape()}")
    softmaxed = ElementwiseMulNode([
        exp, rowsum_expanded_inverted
    ])
...
def cross_entropy_node(dep: TensorNode, target_distributions: np.ndarray):
    ...
    mul_node = ElementwiseCrossLogNode(target_distributions_node, dep)
    rowsum = TensorDotNode(mul_node, ConstantNode(np.ones(shape[-1])), 1)
    neg_node = ElementwiseUnaryNode(rowsum, elementwise.CommonElementwiseOperations.mulby(-1.0))
    return neg_node
...
def avg_node(dep: TensorNode, axis: int):
    ...
    return ElementwiseUnaryNode(sum_nd, elementwise.CommonElementwiseOperations.mulby(1.0 / dep.get_shape()[axis]))

def mse_node(dep: TensorNode, target: np.ndarray):
    neg_target_node = ConstantNode(-target)
    add_node = ElementwiseAddNode([dep, neg_target_node])
    sq_node = ElementwiseUnaryNode(add_node, elementwise.CommonElementwiseOperations.ipow(2))
